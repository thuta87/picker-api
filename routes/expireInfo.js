    ...
    oracledb.getConnection(
        ...
        function(err, connection){
            ...
            connection.execute("SELECT ARTCEXR    AS ART_CODE,"+
                                ...
                                "  NVL(PKSTOCK.GETSTOCKDISPOENQTE(1,(select asisite FROM adm_sites where asiuser in (SELECT aususer FROM adm_users where AUSEXTUSER like :uname)"+
                                "   AND (asisite not like '20%' AND asisite not like '12%' AND asisite not like '13%')"+
                                "   AND ASIAPPLI like 'GSM' "+
				" Union All "+
				" select TO_NUMBER(site) as asisite from INTERF_IN.PCK_SITES "+
				" where site like '20%'"+
				" AND user_id like :uname"+
				"	), ARTUL.ARUCINL),0) AS ART_QTY,"+
                                "  NULL AS REQ_QTY "+
                                " FROM ARTRAC,"+
                                "  ARTUL,"+
                                "  ARTVL,"+
                                ...
                                " AND PKSTOCK.GETSTOCKDISPOENQTE(1,(select asisite FROM adm_sites where asiuser in (SELECT aususer FROM adm_users where AUSEXTUSER like :uname)"+
                                "   AND (asisite not like '20%' AND asisite not like '12%' AND asisite not like '13%')"+
                                "   AND ASIAPPLI like 'GSM' "+
                                " Union All "+
                                " select TO_NUMBER(site) as asisite from INTERF_IN.PCK_SITES "+
                ...
                function(err, results){
                    ...
                    connection.release(function(err) {
                        if (err) {
                            ...
                            console.log("GET /expire_info/?barcode=" + req.query.barcode + "&uname=" + req.query.uname+": Connection released");    
                        }                        
                    });
                }
            );
